Hooks.on("init", () => {
    ...
    game.settings.registerMenu("dgr-harms-way", "init-dialog", {
        ...
        label : "Initialize",
        hint : "This will import the content from the Degenesis Harm's Way Adventure Module",
...
class DGRHarmsWayInitialization extends Dialog {
    ...
    async initializeEntities() {
        ...
        let packList= [ `${this.moduleKey}.dgr-harms-way-actors`,
                    `${this.moduleKey}.dgr-harms-way-journals`,
                    `${this.moduleKey}.dgr-harms-way-items`]
...
class DGRHarmsWayInitializationSetup {
    ...
    static async displayFolders() {
        ...
        game.folders.entities.forEach(async f => {
            if (f.data.parent)
                await f.setFlag("dgr-harms-way", "initialization-parent", game.folders.get(f.data.parent).data.name)
        })
    ...
    static async setFolderFlags() {
        for (let scene of game.scenes.entities)
            await scene.update({"flags.dgr-harms-way" : { "initialization-folder" : game.folders.get(scene.data.folder).data.name, sort : scene.data.sort}})
        for (let actor of game.actors.entities)
            await actor.update({"flags.dgr-harms-way" : { "initialization-folder" : game.folders.get(actor.data.folder).data.name, sort : actor.data.sort}})
        for (let item of game.items.entities)
            await item.update({"flags.dgr-harms-way" : { "initialization-folder" : game.folders.get(item.data.folder).data.name, sort : item.data.sort}})
        for (let journal of game.journal.entities)
            await journal.update({"flags.dgr-harms-way" : { "initialization-folder" : game.folders.get(journal.data.folder).data.name, sort : journal.data.sort}})
    }

    static async setSceneNotes() {
        for (let scene of game.scenes.entities)
            if (scene.data.journal)
                await scene.setFlag("dgr-harms-way", "scene-note", game.journal.get(scene.data.journal).data.name)
    }
    ...
    static async setEmbeddedEntities() {
        for (let scene of game.scenes.entities)
        {
            ...
            {
                setProperty(note, "flags.dgr-harms-way.initialization-entryName", game.journal.get(note.entryId).data.name)
            }
            let tokens = duplicate(scene.data.tokens)
            for (let token of tokens)
            {
                setProperty(token, "flags.dgr-harms-way.initialization-actorName", game.actors.get(token.actorId).data.name)
            }
